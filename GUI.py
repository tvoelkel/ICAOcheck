...
class myUI(Frame, metaclass=Singleton):
    ...
    def __init__(self):
        ...
        self.checkBox = Checkbutton(self,
                                    text="wenn möglich nach ICAO-Standard zuschneiden",
                                    command=self.check_cut)
        self.checkBox.grid(row=3, column=2, columnspan=3, sticky=W)
        self.checkBoxF = Checkbutton(self,
                                     text="Bilder mit Facial Landmarks speichern",
    ...
    def check_images(self):
        ...
        if self.file_list != []:
            ...
            for image in self.file_list:
                #run helper function to set the facial landmark array for each image
                #and set a flag whether the number of faces != 1
                (image.facial_landmarks, image.facial_landmarks_error) = getFacialLandmarks(
    ...
    def switchDisplayedImage(self, imageOffset):
        if (
                (self.currentDisplayedResult + imageOffset) <= len(self.file_list)
