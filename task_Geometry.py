...
def checkGeometry(imagelist, Check_Cut):
    ...
    #generate paths for textfile, resultfolder etc
    ...
    for image in imagelist:
        #start function if number of faces =1
    ...
    #write result in textfile
    with open(txtfile_path, "a") as fh:
        fh.write(
            "Bilder ICAO konform: " + str(n_konform)
...
def _checkGeometry(image, shape):
    #shape[n][m]: n is the facial landmark from 0 to 67,
    #             m is the pixel-coordinate (0 = x-value, 1 = y-value)
    ...
    leftEyeCenter = (
        int((shape[43][0] + shape[44][0] + shape[46][0] + shape[47][0]) / 4),
        int((shape[43][1] + shape[44][1] + shape[46][1] + shape[47][1]) / 4)
    )

    rightEyeCenter = (
        int((shape[37][0] + shape[38][0] + shape[40][0] + shape[41][0]) / 4),
        int((shape[37][1] + shape[38][1] + shape[40][1] + shape[41][1]) / 4)
    )
    ...
    headwidth_W = int(math.sqrt(((shape[16][0] - shape[0][0])**2)
                                + ((shape[16][1] - shape[0][1])**2)))
    ...
    #proof all conditions
    proof(
        image_data,
        imagewidth_A,
        imageheight_B,
        horizontaldistance_Mh,
        verticaldistance_Mv,
        headwidth_W,
        headlength_L,
        difference_allowed,
    ...
    #image is ICAO compliant, if all conditions true,
    if (
            image_ratio == True
            and horizontal_ratio == True
            and vertical_ratio == True
            and headwidth_ratio == True
            and headlength_ratio == True
            and head_roll == True
    ):
    ...
    if (
            ...
            or vertical_ratio == False
            or headwidth_ratio == False
            or headlength_ratio == False
            or head_roll == False
    ):
        cut_img = cut(
            image_data,
        ...
        if cut_img == True:
            ...
            if check_cut == 1:
                cv2.imwrite(
                    final_path + "\\" + image.image_name[:-4] + "-cut.jpg",
                    cutted_img, [int(cv2.IMWRITE_JPEG_QUALITY), 100]
                )
        else:
            ausgabe = ausgabe + " \n--> Zuschnitt nicht möglich"
            n_nzuschnitt = n_nzuschnitt + 1

    #write the result of test and cut in result-string
